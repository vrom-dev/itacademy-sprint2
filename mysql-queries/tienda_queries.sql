...
SELECT nombre, precio AS 'precio (€)', round(precio*1.19, 2) AS 'precio ($)' FROM producto;
...
SELECT upper(nombre) AS 'nom de producto', precio AS 'precio (€)' FROM producto;

-- #7 Llista els noms i els preus de tots els productos de la taula producto, convertint els noms a minúscula.
SELECT lower(nombre) AS 'nom de producto', precio AS 'precio (€)' FROM producto;

-- #8 Llista el nom de tots els fabricants en una columna, i en una altra columna obtingui en majúscules els dos primers caràcters del nom del fabricant.
SELECT nombre, upper(substring(nombre, 1, 2)) as 'codename' FROM fabricante;
...
SELECT p.nombre AS 'producto', p.precio, f.nombre AS 'fabricante' FROM producto AS p JOIN fabricante AS f ON p.codigo_fabricante = f.codigo;
...
-- #22 Retorna una llista amb el nom del producte, preu i nom de fabricant de tots els productes de la base de dades. Ordeni el resultat pel nom del fabricador, per ordre alfabètic.
 SELECT p.nombre AS 'producto', p.precio, f.nombre AS 'fabricante' FROM producto AS p JOIN fabricante AS f ON p.codigo_fabricante = f.codigo ORDER BY f.nombre; 

-- #23 Retorna una llista amb el codi del producte, nom del producte, codi del fabricador i nom del fabricador, de tots els productes de la base de dades.
 SELECT p.nombre AS 'producto', p.codigo AS 'codigo producto' , f.nombre AS 'fabricante', f.codigo AS 'codigo fabricante' FROM producto AS p JOIN fabricante AS f ON p.codigo_fabricante = f.codigo;

-- #24 Retorna el nom del producte, el seu preu i el nom del seu fabricant, del producte més barat.
SELECT p.nombre AS 'producto', p.precio, f.nombre AS 'fabricante' FROM producto AS p JOIN fabricante AS f ON p.codigo_fabricante = f.codigo ORDER BY p.precio LIMIT 1;

-- #25 Retorna el nom del producte, el seu preu i el nom del seu fabricant, del producte més car.
SELECT p.nombre AS 'producto', p.precio, f.nombre AS 'fabricante' FROM producto AS p JOIN fabricante AS f ON p.codigo_fabricante = f.codigo ORDER BY p.precio DESC LIMIT 1;
...
SELECT * FROM producto AS p JOIN fabricante AS f ON p.codigo_fabricante = f.codigo WHERE f.nombre = 'Asus' OR f.nombre = 'Hewlett-Packard' OR f.nombre = 'Seagate';
...
SELECT p.nombre, p.precio, f.nombre AS 'fabricante' FROM producto AS p JOIN fabricante AS f ON p.codigo_fabricante = f.codigo  WHERE f.nombre LIKE '%e';

-- #31 Retorna un llistat amb el nom i el preu de tots els productes el nom de fabricant dels quals contingui el caràcter w en el seu nom.
SELECT p.nombre, p.precio, f.nombre AS 'fabricante' FROM producto AS p JOIN fabricante AS f ON p.codigo_fabricante = f.codigo  WHERE f.nombre REGEXP 'w';
...
SELECT DISTINCT f.codigo, f.nombre FROM fabricante AS f JOIN producto AS p ON p.codigo_fabricante = f.codigo;

-- #34 Retorna un llistat de tots els fabricants que existeixen en la base de dades, juntament amb els productes que té cadascun d'ells. El llistat haurà de mostrar també aquells fabricants que no tenen productes associats.
SELECT f.codigo AS 'codigo fabricante', f.nombre AS 'fabricante', p.nombre AS 'producto' FROM fabricante AS f LEFT JOIN producto AS p ON p.codigo_fabricante = f.codigo;

-- #35 Retorna un llistat on només apareguin aquells fabricants que no tenen cap producte associat.
SELECT f.codigo AS 'codigo fabricante', f.nombre AS 'fabricante', p.nombre AS 'producto' FROM fabricante AS f LEFT JOIN producto AS p ON p.codigo_fabricante = f.codigo WHERE p.codigo IS NULL;

-- #36 Retorna tots els productes del fabricador Lenovo. (Sense utilitzar INNER JOIN).
SELECT * FROM producto AS p WHERE p.codigo_fabricante IN (SELECT f.codigo FROM fabricante AS f WHERE f.nombre = 'Lenovo');
...
SELECT * FROM producto AS p JOIN fabricante as f ON p.codigo_fabricante = f.codigo WHERE f.nombre = 'Asus' AND p.precio > (SELECT avg(precio) FROM producto AS p JOIN fabricante AS f ON p.codigo_fabricante = f.codigo WHERE f.nombre = 'Asus');